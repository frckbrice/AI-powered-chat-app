		...
		<>
			<div className={`flex gap-2 items-center p-3 hover:bg-chat-hover cursor-pointer `}>
					...
					{conversation.isOnline && (
                        ...
                        <AvatarImage src={conversationImage || "/placeholder.png"} alt={conversationName} className='object-cover rounded-full' />
					<AvatarFallback>
				...
				<div className='w-full'>
                    ...
                    <p className='text-[12px] mt-1 text-gray-500 flex items-center gap-1 '>
                        {lastMessage?.sender === authUser?._id ? <MessageSeenSvg /> : null}
                        {conversation.isGroup && <Users size={16} />}
                        {!lastMessage && "Say Hi!"}
                        {lastMessageType === "text" && (lastMessage?.content?.length ?? 0) > 30 ? (
